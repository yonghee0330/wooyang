...
- 관리자 기능: 관리자 권한 사용자 전용 결재 대기 처리, 전체 신청 이력 조회/CSV 다운로드, 직원별 연차 현황 조회

## 관리자 기능

- `admin.html`: 결재 대기 신청을 카드 형태로 확인하고 승인 또는 반려 처리
- `admin-history.html`: 전체 휴가 신청 이력을 상태/휴가종류/신청자 기준으로 필터링하고 UTF-8 BOM 포함 CSV 다운로드
- `admin-employees.html`: 전체 직원 수, 평균 사용/잔여 연차, 직원별 잔여비율 프로그레스 바 확인
- 권한이 `관리자`인 사용자만 접근할 수 있으며, 비관리자가 URL로 직접 접근하면 대시보드로 이동합니다.
...
4. 관리자 권한 사용자는 대시보드 헤더에 `관리자 페이지` 버튼이 표시되는지 확인합니다.
5. `apply.html`에서 휴가종류와 기간을 입력하고 사용일수가 자동 계산되는지 확인합니다.
6. 휴가 신청 제출 후 접수 토스트가 표시되고 대시보드로 돌아오는지 확인합니다.
7. `admin.html`에서 결재 대기 목록 조회, 승인, 반려 사유 입력 모달을 확인합니다.
8. `admin-history.html`에서 필터와 CSV 다운로드 파일명을 확인합니다.
9. `admin-employees.html`에서 부서 필터, 요약 카드, 잔여비율 프로그레스 바를 확인합니다.
10. 비관리자 계정으로 관리자 URL 직접 접근 시 대시보드로 이동하는지 확인합니다.
11. 로그아웃 후 보호 페이지 접근 시 로그인 화면으로 이동하는지 확인합니다.
